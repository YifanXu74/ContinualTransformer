...
def get_wds_dataset(args, collate_fn = None):
    # transform
    if args.force_vae:
        preprocess_img = DataAugmentationForContinualTransformer(args)
    else:
        preprocess_img = SimpleDataAugmentationForContinualTransformer(args)
    # always
    is_train = True
    # not sure
    epoch=0
    floor=False
    # data_path = "/data/LAION-400M/{00000..41455}.tar"
...
def simple_webdataset_collate_fn(batch):
    image_list = batch[0]
...
if __name__ == "__main__":
    ...
    """
    """
    torchrun --nnodes=1 --nproc_per_node=1 main_pretrain_cook.py \
    --exp_name image_text_itc \
    --model vlmo_base_patch16 \
